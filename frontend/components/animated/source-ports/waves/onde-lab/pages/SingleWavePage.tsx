...
import { EndCondition, WaveMode, WaveMetrics, TracerData } from '../physics/WaveEngine'
import { useTracerHistory } from '../hooks/useTracerHistory'
import { useTheme } from '../context/ThemeContext'
import { useKeyboardShortcuts } from '../hooks/useKeyboardShortcuts'

interface SingleWavePageProps {
    onNavigate: (page: 'single' | 'interference' | 'longitudinal' | 'multimedium' | 'circular') => void;
}

export default function SingleWavePage({ onNavigate }: SingleWavePageProps) {
    ...
    const [linearMass, setLinearMass] = useState(1.0)
    ...
    const handleTracerUpdate = useCallback((tracer1: TracerData | null, tracer2: TracerData | null) => {
        ...
        if (tracer1 && tracer2 && metrics && metrics.waveSpeed > 0) {
            const distPx = Math.abs(tracer2.x - tracer1.x);
    ...
    const boundaryInfo: Record<EndCondition, { title: string; desc: string }> = {
        fixed: { title: 'Fixe', desc: 'L\'onde se réfléchit avec inversion de phase (180°)' },
        loose: { title: 'Libre', desc: 'L\'onde se réfléchit sans changement de phase' },
    ...
    const accentBg = isDark ? 'bg-[#FBAE17]' : 'bg-[#FBAE17]';
                ...
                <div className={`flex-1 flex flex-col ${canvasBg}`}>
                    ...
                    {showGraph && showTracerPoints && (
                        <DisplacementGraph
                            ...
                            maxAmplitude={Math.max(80, amplitude * 1.2)}
                            timeWindow={6}
                            isPlaying={isPlaying}
                            height={200}
                            theme={theme}
                ...
                <aside className={`w-80 ${cardBg} border-l ${borderColor} overflow-y-auto p-4 space-y-4`}>
                    ...
                    <section className={`${isDark ? 'bg-[#334155]/50' : 'bg-[#FFF9EB]'} rounded-2xl p-4 space-y-3 border ${isDark ? 'border-[#475569]' : 'border-[#FBAE17]/20'}`}>
                        ...
                        <p className={`text-xs ${textTertiary}`}>
                            {waveMode === 'manual' 
                                ? 'Cliquez sur le bouton ou glissez le point source' 
                                : 'Génération continue d\'onde sinusoïdale'}
                        </p>
                    </section>
                    ...
                    <section className={`${isDark ? 'bg-[#334155]/50' : 'bg-white'} rounded-2xl p-4 space-y-4 border ${borderColor}`}>
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between text-sm">
                                <span className={textPrimary}>Masse Linéique</span>
                                <span className={`${accentLight} font-mono font-medium`}>{linearMass.toFixed(1)} g/m</span>
                            </div>
                            <input
                                type="range" min="0.5" max="3.0" step="0.1"
                                value={linearMass}
                                onChange={(e) => setLinearMass(Number(e.target.value))}
                                className="w-full accent-[#FBAE17]"
                            />
                            <p className={`text-xs ${textTertiary}`}>Haute masse = ondes plus lentes (v = √T/μ)</p>
                        </div>
                    ...
                    <section className={`${isDark ? 'bg-[#334155]/50' : 'bg-white'} rounded-2xl p-4 space-y-3 border ${borderColor}`}>
                        ...
                        {showTracerPoints && (
                            <p className={`text-xs ${textTertiary} mt-2`}>
                                Glissez les points vert et violet sur la corde
                            </p>
                        )}
                    </section>
                        ...
                        <section className={`${isDark ? 'bg-gradient-to-br from-[#FFF9EB]/10 to-[#FBAE17]/5' : 'bg-gradient-to-br from-[#FFF9EB] to-[#FEF3C7]'} rounded-2xl p-4 space-y-3 border ${isDark ? 'border-[#FBAE17]/30' : 'border-[#FBAE17]/20'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    ...
                                    { label: 'Amp Max', value: `${metrics.maxAmplitude.toFixed(1)} cm` },
                                ].map(({ label, value }) => (
                    ...
                    <section className={`${isDark ? 'bg-[#334155]/30' : 'bg-[#F8FAFC]'} rounded-2xl p-4 border ${borderColor}`}>
                        ...
                        <p className={`text-xs ${textTertiary} leading-relaxed`}>
                            Le graphique montre le mouvement vertical de deux points sur la corde. 
                            <span className="text-green-500"> Vert (A)</span> et 
                            <span className="text-purple-500"> Violet (B)</span> montrent le mouvement transversal.
                        </p>
                    </section>
                </aside>
            </main>
