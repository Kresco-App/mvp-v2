...
const cmiReturnPrimaryButtonClass = 'min-h-10 rounded-xl bg-kresco px-6 py-3 font-bold text-white transition-[box-shadow,opacity,transform] duration-150 ease-out hover:opacity-90 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#5b60f9]/20 motion-reduce:transition-none motion-reduce:active:scale-100'
const cmiReturnSecondaryButtonClass = 'min-h-10 rounded-xl bg-slate-200 px-6 py-3 font-bold text-slate-700 transition-[background-color,box-shadow,transform] duration-150 ease-out hover:bg-slate-300 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-white/15 motion-reduce:transition-none motion-reduce:active:scale-100'
  ...
  return (
    <main className="min-h-screen bg-slate-900 flex items-center justify-center px-4">
      <section className="max-w-sm text-center">
        ...
        {status === 'loading' && (
          <>
            <Loader2 size={40} className="animate-spin text-kresco mx-auto mb-4 motion-reduce:animate-none" />
        ...
        {status === 'active' && (
          <>
            ...
            <button type="button" onClick={() => router.push('/home')} className={cmiReturnPrimaryButtonClass}>
              Commencer
            </button>
          </>
        )}
        ...
        {status === 'pending' && outcome === 'ok' && (
          <>
            <Loader2 size={40} className="animate-spin text-amber-400 mx-auto mb-4 motion-reduce:animate-none" />
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              <button type="button" onClick={() => setRefreshNonce((value) => value + 1)} className={cmiReturnSecondaryButtonClass}>
                Actualiser
              </button>
            </div>
          </>
        ...
        {outcome === 'fail' && (
          <>
            ...
            <button type="button" onClick={() => router.push('/pricing')} className={cmiReturnPrimaryButtonClass}>
              Retour aux tarifs
            </button>
          </>
        )}
