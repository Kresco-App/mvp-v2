...
test('topic workspace hydrates with mocked course data', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'Topic note' })).toHaveCount(0)
  ...
  const expandedWhiteboard = page.getByRole('dialog', { name: /Continuity introduction whiteboard/i })
  await expect(expandedWhiteboard).toBeVisible()
  const expandedBox = await expandedWhiteboard.boundingBox()
  const viewport = page.viewportSize()
  expect(expandedBox).not.toBeNull()
  expect(viewport).not.toBeNull()
  expect(Math.round(expandedBox!.width)).toBeLessThanOrEqual(viewport!.width - 16)
  expect(Math.round(expandedBox!.height)).toBeLessThanOrEqual(viewport!.height - 16)
  expect(Math.round(expandedBox!.width)).toBeGreaterThan(viewport!.width * 0.65)
  expect(Math.round(expandedBox!.height)).toBeGreaterThan(viewport!.height * 0.75)
  await expect(page.getByText('Click outside or press Esc to return')).toBeVisible()
  await page.getByTestId('whiteboard-expanded-backdrop').click({ position: { x: 4, y: 4 } })
  await expect(expandedWhiteboard).toBeHidden()
  await page.getByRole('button', { name: 'Expand' }).click()
  await expect(expandedWhiteboard).toBeVisible()
