...
def upgrade() -> None:
    ...
    if "staff_payment_profiles" not in tables:
        op.create_table(
            ...
            sa.Column("allowed_template_ids_json", sa.JSON(), nullable=False, server_default=sa.text("'[]'")),
            sa.Column("metadata_json", sa.JSON(), nullable=False, server_default=sa.text("'{}'")),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
            sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    ...
    if "redemption_code_templates" not in tables:
        op.create_table(
            ...
            sa.Column("subject_ids_json", sa.JSON(), nullable=False, server_default=sa.text("'[]'")),
    ...
    if "staff_payment_requests" not in tables:
        op.create_table(
            ...
            sa.UniqueConstraint("redemption_code_id", name="uq_staff_payment_requests_redemption_code"),
