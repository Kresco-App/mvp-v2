...
async def _breakdowns(
    ...
    breakdowns = await _gather_reads(
        *(lambda session, model=model, column=column: _breakdown(session, model, column) for _, model, column in specs)
    )
    return {key: breakdown for (key, _, _), breakdown in zip(specs, breakdowns)}
...
async def _gated_content_rollups(db: AsyncSession) -> tuple[dict[str, int], dict[str, dict[str, int]], dict[str, dict[str, int]]]:
    ...
    for key, model, has_free_preview in GATED_CONTENT_MODELS:
        ...
        operations.extend(
            [
                lambda session, model=model: _breakdown(session, model, model.required_tier),
                lambda session, model=model: _breakdown(session, model, model.required_feature_key),
            ]
        )
        ...
        if has_free_preview:
            ...
            breakdown_offset = 3
        by_required_tier[key] = results[breakdown_offset]
        by_feature[key] = results[breakdown_offset + 1]
...
async def _ops_readiness(
    ...
    *,
    ...
    (
        ...
        lambda session: _breakdown(session, Resource, Resource.resource_type),
        lambda session: _breakdown(session, Resource, Resource.provider),
...
async def get_admin_overview(
    ...
    users_by_role, users_by_pro_status, entitlements_by_status, entitlements_by_source = await _gather_reads(
        lambda session: _breakdown(session, User, User.role),
        lambda session: _breakdown(session, User, User.is_pro),
        lambda session: _breakdown(session, UserSubjectEntitlement, UserSubjectEntitlement.status),
        lambda session: _breakdown(session, UserSubjectEntitlement, UserSubjectEntitlement.source),
    )
    access_billing = {
        "users_by_role": users_by_role,
        "users_by_pro_status": users_by_pro_status,
        "entitlements_by_status": entitlements_by_status,
        "entitlements_by_source": entitlements_by_source,
        "policy": {
    ...
    (
        ...
        xp_transactions_by_reason,
        daily_quests_by_completion,
        total_xp,
        average_xp_per_user_with_xp,
        average_streak_days,
    ) = await _gather_reads(
        lambda session: _breakdown(session, LessonProgress, LessonProgress.status),
        lambda session: _breakdown(session, TopicItemProgress, TopicItemProgress.status),
        lambda session: _breakdown(session, ContentProgress, ContentProgress.item_type),
        lambda session: _breakdown(session, XPTransaction, XPTransaction.reason),
        lambda session: _breakdown(session, DailyQuest, DailyQuest.completed),
    ...
    progress_xp = {
        "lesson_progress_by_status": lesson_progress_by_status,
        "topic_item_progress_by_status": topic_item_progress_by_status,
        "content_progress_by_type": content_progress_by_type,
        "xp_transactions_by_reason": xp_transactions_by_reason,
        "daily_quests_by_completion": daily_quests_by_completion,
        "total_xp": total_xp,
    ...
    (
        ...
        lambda session: _breakdown(session, ExamProblem, ExamProblem.difficulty),
    ...
    events_by_type, upcoming_events, live_events = await _gather_reads(
        lambda session: _breakdown(session, CalendarEvent, CalendarEvent.event_type),
    ...
    (
        ...
        lambda session: _breakdown(session, ActivityEvent, ActivityEvent.event_type),
        lambda session: _breakdown(session, ActivityEvent, ActivityEvent.target_type),
    ...
    engagement = {
        ...
        "activity_events_7d": activity_events_7d,
        "activity_events_by_type": activity_events_by_type,
        "activity_events_by_target": activity_events_by_target,
        "quiz_attempts": quiz_attempts_total,
    ...
    saves_by_target, comments_with_topic_item, notes_with_topic, notes_with_topic_item = await _gather_reads(
        lambda session: _breakdown(session, SavedItem, SavedItem.target_type),
    ...
    notifications_unread, notifications_by_type, notifications_by_read_status, notifications_created_7d = await _gather_reads(
        lambda session: _count(session, Notification, Notification.is_read == False),  # noqa: E712
        lambda session: _breakdown(session, Notification, Notification.type),
        lambda session: _breakdown(session, Notification, Notification.is_read),
    ...
    notifications = {
        ...
        "by_read_status": notifications_by_read_status,
        "created_7d": notifications_created_7d,
    }

    admin_audit_created_7d, admin_audit_by_action, admin_audit_by_model = await _gather_reads(
        lambda session: _count(session, AdminAuditLog, AdminAuditLog.created_at >= recent_since),
        lambda session: _breakdown(session, AdminAuditLog, AdminAuditLog.action),
        lambda session: _breakdown(session, AdminAuditLog, AdminAuditLog.model_name),
    )
    admin_audit = {
        "total": totals["admin_audit_logs"],
        "created_7d": admin_audit_created_7d,
        "by_action": admin_audit_by_action,
        "by_model": admin_audit_by_model,
    }
