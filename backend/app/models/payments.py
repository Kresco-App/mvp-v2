...
class ManualAccessGrant(Base):
    ...
    status: Mapped[str] = mapped_column(String(40), nullable=False, default="completed", server_default="completed")
    entitlement_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("user_subject_entitlements.id", ondelete="SET NULL"), nullable=True, index=True)
...
class RefundRequest(Base):
    ...
    requested_by_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    reviewed_by_user_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
